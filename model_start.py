...
class GMT_Zero_L(L.LightningModule):
    def __init__(self):
        super().__init__()
...
model = GMT_Zero_L.load_from_checkpoint(checkpoint_path="./model_weight/GMT_Zero.ckpt",strict=False).to(device)
...
def generate(tokenizer,input_ids,T,P,max_len:int=100):
    ...
    for i in range(max_len):
        ...
        input_ids = indices.gather(-1, select_ids)
        current_sequence = torch.cat([current_sequence, input_ids], dim=-1)

        if input_ids == tokenizer.eos_token_id:
            break
...
while True:
    ...
    input_ids = tokenizer.encode(inference_text, return_tensors="pt",add_special_tokens=True).to(device)
